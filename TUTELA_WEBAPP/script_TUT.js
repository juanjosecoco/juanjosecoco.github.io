...
y += 10;
doc.text('JUEZ DE LA REPUBLICA (REPARTO) \n E.S.D', 10, y);
doc.text('ASUNTO: ACCIÓN DE TUTELA POR OMISION A DERECHO DE PETICIÓN', 10, y += 20);
doc.text('REFERENCIA: PRESENTACION DE SOLICITUD', 10, y += 10);
doc.setFontStyle('normal');
doc.text(nombre + ' mayor de edad, identificado con documento no. ' + identificacion + ' interpongo la presente ACCION DE TUTELA, en contra de ' + destinatario + ', fundamentada en los siguientes:', 10, y += 15,  { maxWidth: 180 });
doc.setFontStyle('bold');
doc.text('HECHOS:', 10, y += 25);
doc.setFontStyle('normal');
doc.text('PRIMERO: ' + hechos, 10, y += 10,  { maxWidth: 180 });
doc.text('SEGUNDO: La omisión en la respuesta de la petición ha generado las siguientes consecuencias: ' + peticion, 10, y += 15,  { maxWidth: 180 });
doc.setFontStyle('bold');
doc.text('PETICION:', 10, y += 30);
doc.setFontStyle('normal');
doc.text('PRIMERA: Que se declare que ' + destinatario + ' ha vulnerado mi derecho fundamental de pretición, consagrado en el artículo 23 de la Constitución Política de 1991.', 10, y += 10,   { maxWidth: 180 });
doc.text('SEGUNDA: Que se proteja mi derecho fundamental de petición y se ordene a ' + destinatario + ' dar respuesta de fondo a mi solicitud, dentro de las 48 horas siguientes a la notificación del fallo de la presente tutela.', 10, y += 25,   { maxWidth: 180 });
doc.setFontStyle('bold');
doc.text('ANEXOS:', 10, y += 30);
doc.setFontStyle('normal');
doc.text(anexos, 10, y += 10);
doc.setFontStyle('bold');
doc.text('NOTIFICACIONES:', 10, y += 15);
doc.setFontStyle('normal');
doc.text(notificaciones, 10, y += 10);
doc.text('Atentamente,\n\n' + nombre + '\n' + identificacion, 10, y += 10);
...
for (var i = 1; i <= totalPages; i++) {
    ...
    var str = "Hecho con TUTELA GENERATOR";
